function setHouse(a) {
    game.house = parseFloat(parseFloat(a).toFixed(2)), houseSave = game.house, "undefined" != $.cookie("gameHouse") && $.removeCookie("gameHouse"), $.cookie("gameHouse", houseSave)
}
...
﻿function setOdds(a) {
    game.odds = parseFloat(parseFloat(a).toFixed(2)), oddsSave = game.Odds, "undefined" != $.cookie("gameOdds") && $.removeCookie("gameOdds"), $.cookie("gameOdds", oddsSave)
}

﻿function setMulti(a) {
    game.multi = parseFloat(parseFloat(a).toFixed(2)), multiSave = game.Multi, "undefined" != $.cookie("gameMulti") && $.removeCookie("gameMulti"), $.cookie("gameMulti", multiSave)
}
...
function onloadCallback() {
    grecaptcha.render("faucetClaimCaptcha", {
        sitekey: "6LdL7wsUAAAAAGlsLscdL2rnKZbPwWdwHjFHBBzf",
...
function startNewGame() {
  // APP ID
    if (!connected) return $("#theDimmer").html('Error: Not connected!<br><a href="https://www.moneypot.com/oauth/authorize?app_id=2433&response_type=token&state=Meh&redirect_uri=https://cooldice7.github.io">Click here to login</a>'), void $("#game_left").dimmer("show");
...
{
var _settings = {
    baseBet: game.bet, // 1 bit. Cannot be lower than 0.01.
    housez:  game.house * 0.01,
    multiply: game.multi, // Multiply the previous stake by 8 times if lost.
    tiles: 1, // Amount of tiles to click before cashing out.
};
 
// --- Edit over this line --- \\
var bet = _settings.baseBet, lastBet = 0, lastGame = null;
var houseEdge =_settings.housez, odds, next;
var tilesClicked = 0;
...
function placeBet(){
    ...
    odds = ((1.0-houseEdge) / game.odds );
    var random= Math.floor(Math.random() * (((0.99-odds) * Math.pow(2,32)) - 1) + 1);
    var wager = (bet*100);
    var range = Math.floor(Math.pow(2,32)*(odds));
	var rangeWin = (range + random);
	var rangestart = (random + 0);
    ...
    $.ajax({
        ...
        data: JSON.stringify({
            ...
            wager: wager,
            "payouts": [
                {from: rangestart, to: rangeWin, value: ((bet+next)*100)},
	    ]
        }),
        dataType: "json",
        ...
        if(data.outcome >= rangeWin || data.outcome <= rangestart ){ // lost
...
$("#bet").val(game.bet), $("#bombsButtons .button").each(function(a) {
    ...
    var a = "https://www.moneypot.com/dialog/deposit?app_id=2433",
        b = "manage-auth",
        c = "width=420,height=350,left=100,top=100",
        d = window.open(a, b, c);
    d.focus()
}), $("#withdrawButton").click(function() {
    var a = "https://www.moneypot.com/dialog/withdraw?app_id=2433",
        b = "manage-auth",
        c = "width=420,height=350,left=100,top=100",
        d = window.open(a, b, c);
    d.focus()
